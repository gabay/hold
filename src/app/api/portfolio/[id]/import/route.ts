...
export async function POST(
  ...
  try {
    ...
    const priceIdx = headers.indexOf("pricepershare") !== -1 ? headers.indexOf("pricepershare") : headers.indexOf("price");
    const currencyIdx = headers.indexOf("currency");
    const feeIdx = headers.indexOf("fee");
    const dateIdx = headers.indexOf("transactiondate") !== -1 ? headers.indexOf("transactiondate") : headers.indexOf("date");
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      let currency = currencyIdx !== -1 && row[currencyIdx] ? row[currencyIdx].toUpperCase() : null;
      const fee = feeIdx !== -1 && row[feeIdx] ? parseFloat(row[feeIdx]) : 0;
      const transactionDate = dateIdx !== -1 && row[dateIdx] ? new Date(row[dateIdx]) : new Date();

      if (isNaN(quantity) || isNaN(pricePerShare) || (type !== "BUY" && type !== "SELL")) {
        return NextResponse.json({ error: `Invalid numeric or action data at row ${i + 1}` }, { status: 400 });
      }
      ...
      if (!currency) {
        try {
          ...
          return NextResponse.json({ error: `Could not resolve ticker symbol "${symbol}" at row ${i + 1}` }, { status: 400 });
        }
      }
